...
func jump():
	if Input.is_action_pressed("ui_up"): #If the jump button is pressed the jumpcheck is set to 1, and the up vector is applied to the glib.
		if(is_on_floor()):
			move_and_collide(up)
		jumpcheck = 1
func _physics_process(delta):  
	...
	if(!is_on_floor() && jumpcheck == 1): #If the player if off the ground, and had pressed the jump button, the up vector is applied to glib, while also in the opposite direction of the gravity vector, which is increasing.
		move_and_collide(up)
	if(is_on_floor()): #When player lands, gravity and jumpcheck are both reset.
		jumpcheck = 0
		gravity.y = 1